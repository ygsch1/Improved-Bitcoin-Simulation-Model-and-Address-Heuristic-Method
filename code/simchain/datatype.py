...
class Vin(tuple):
    ...
    def __new__(cls,to_spend,signature,pubkey,sequence,witness):
        return super(cls,Vin).__new__(cls,(to_spend,signature,pubkey,sequence,witness))
    ...
    def __repr__(self):
        return "Vin(to_spend:{0},signature:{1},pubkey:{2},sequence:{3},witness:{4})".format(self[0],self[1],self[2],self[3],self[4])
    

#value output for a transaction
class Vout(tuple):
    
    def __new__(cls,to_addr,value,addr_type):
        #return super(Vout,cls).__new__(cls,(to_addr,value))
        #return super(Vout,cls).__new__(cls,(to_addr,round(value,3)))
        return super(Vout, cls).__new__(cls, (to_addr, value,addr_type))
    ...
    def __repr__(self):
        return "Vout(to_addr:{0},value:{1},addr_type:{2})".format(self[0],self[1],self[2])
    ...
    def create_coinbase(cls, pay_to_addr, value):
        return cls(
            ...
            tx_out = [Vout(to_addr=pay_to_addr,
                           value=value,addr_type=addfunctions.Address_type())]
...
if __name__ == "__main__":
    ...
    p = Pointer(1,2)
    vout = Vout(1,2,s)
    utxo = UTXO((1,2),[2,3],True,1)  #要不要改成utxo = UTXO((1,2,s),[2,3],True,1)
    vin = Vin(p,b'1',b'12',s,s)
    tx = Tx([vin],[vout])
